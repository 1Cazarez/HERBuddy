customElements.define('hb-nav-bar', class extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `
        <nav id="device-nav" class="hidden absolute bottom-0 left-0 right-0 bg-deepIndigo/95 backdrop-blur-md border-t border-white/10 px-2 pt-2.5 pb-[calc(0.625rem+env(safe-area-inset-bottom))] justify-around items-center z-30">
            ...
            <button onclick="switchTab('match')" id="nav-match" class="nav-btn flex flex-col items-center gap-1 text-slate-400 hover:text-neonPink transition">
